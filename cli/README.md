...
The option `dm` can be used to specify a device model name used for node energy consumption calculations. If an invalid device model is set, the default value will be used. For available device models type command `devicemodel`.
...
```shell
energy (txt | csv) "<filename>" [detailed]
```
...
The optional `detailed` flag (only available for `csv`) writes a per-radio-state breakdown per node into a separate `_nodes_detailed.csv` file. Each row represents one radio state, showing the time spent (ms) and energy consumed (mJ) in that state. Radio states with zero time spent are omitted.
...
> energy txt
Done
> energy csv "energy_consumption"
Done
> energy csv "energy_consumption" detailed
Done
```
